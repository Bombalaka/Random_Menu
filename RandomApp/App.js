...
import { getFoods, getUsername } from './utils/api';
...
function Home({ navigation }) {
  ...
  const [username, setUsername] = useState('');
   ...
   // Load Username and Food Count when screen focuses
   useFocusEffect(
    useCallback(() => {
      const loadData = async () => {
        // 1. Get User Name from async storage
        try {
          let name = await AsyncStorage.getItem('username'); 
          console.log("Username from AsyncStorage:", name);
          
          // If not in AsyncStorage, get from backend
          if (!name) {
            console.log("Username not in AsyncStorage, fetching from backend...");
